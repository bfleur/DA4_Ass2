...
data_balanced_long <- data_balanced_ass %>%
  filter(year == "1992" | year == "2016")


data_balanced_long <- data_balanced_long %>%
  arrange(country, year) %>%
  group_by(country) %>%
  mutate(
    d_lnco2_pc_long = lnco2_pc- lag(lnco2_pc),
    d_lngdppc_long = lngdppc- lag(lngdppc),
    d_lnpop_long = lnpop - lag(lnpop),
  ) %>%
  ungroup()

fd_long <- lm_robust(d_lnco2_pc_long ~ d_lngdppc_long + year,
                   data = data_balanced_long, 
                   weights = pop,
                   se_type = "stata", 
                   clusters = country)
...
huxreg("(1)"=fd_long,
       coefs = c("d_lngdppc_long"="d_lngdppc_long",
                 "Year" = "year",
                 "Constant"= "(Intercept)" 
       ),
       statistics = c(N = "nobs", R2 = "r.squared")
...
lags_helper <- paste(paste0("lag(d_lngdppc, ", c(0:2), ")"), collapse = " + ")
lags_helper2 <- paste(paste0("lag(d_electr, ", c(0:2), ")"), collapse = " + ")
...
fd_lm_2_cf <- lm_robust(fd_lm_2_formula_cf,
                     data = data_balanced_ass, 
                     weights = pop,
                     se_type = "stata", 
                     clusters = country
...
huxreg("(1)"=fd_lm_2_cf,
       coefs = c("d_lngdppc"="lag(d_lngdppc, 0)", 
                 ...
                 "d_lngdppc lag1"="lag(d_lngdppc, 1)",
                 "d_lngdppc lag2"="lag(d_lngdppc, 2)",
                 "d_electr" = "lag(d_electr, 0)",
                 "d_electr" = "d_electr",
                 "d_electr lag1" = "lag(d_electr, 1)",
                 "d_electr lag 2" = "lag(d_electr, 2)",
                 "Year" = "year",
                 "Constant"= "(Intercept)" 
       ),
       statistics = c(N = "nobs", R2 = "r.squared")
...
library(texreg)
screenreg(fd_lm_2_cf, caption = "FD with 2-year lags, confounder",
          caption.above = TRUE)
...
huxreg(("Fixed effects model with confounder" = fe_lm2_cf), 
       statistics = c(N = "nobs", R2 = "r.squared"), 
       coefs = c("ln GDP per capita"= "lngdppc", "Year" = "year", 
       "Access to electricity" = "electr"))
